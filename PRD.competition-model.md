...
- **公共内容按区域筛选**：赛事列表、锦标赛列表、新闻列表、日历页、排行榜等公共展示页面，仅展示发布到当前区域的赛事和数据。切换区域后，列表内容随之刷新
...
| 锦标赛/系列赛列表 | 是 | 仅展示 `regions` 包含当前区域且 format 为 championship/series 的 Competition |
...
平台统一采用 `Competition → Round → Stage → Session` 四层结构，不再把"独立赛事"和"锦标赛"作为两套底层模型。前台中文仍可按用户心智展示为"赛事"、"锦标赛"、"预选赛"、"正赛"，但数据结构统一。
...
- 单场赛 = 1 个 Competition + 1 个 Round + 若干 Stage/Session
- 传统锦标赛 = 1 个 Competition + 多个 Round，每个 Round 可只有"正赛日"阶段
- 含预选赛的年度锦标赛 = 1 个 Competition + 多个 Round，每个 Round 内含"预选赛"Stage 和"正赛日/决赛"Stage
...
| format | Enum | 是 | `single` / `championship` / `series` |
...
| cancel_registration_deadline_offset | String | 否 | 默认取消报名截止规则，如"比赛开始前 2 小时" |
| access_requirements_zh / access_requirements_en | String | 否 | 准入条件描述（中/英） |
| rules_zh / rules_en | RichText | 否 | 赛事规则（中/英） |
| scoring_rules_zh / scoring_rules_en | RichText | 否 | 积分规则说明（中/英） |
| scoring_table | ScoringTableEntry[] | 否 | 默认积分表 |
| advancement_rules_zh / advancement_rules_en | RichText | 否 | 晋级/淘汰规则说明（中/英） |
...
| round_number | Integer | 否 | 分站序号，用于排序和展示 |
...
| cover_image | URL | 否 | 分站封面图，未填则使用 Competition 封面 |
...
| stage_ids | UUID[] | 是 | 本 Round 下的 Stage 顺序 |
...
| type | Enum | 是 | `qualifier` / `race_day` / `final` / `consolation` / `practice` / `custom` |
| name_zh / name_en | String | 是（至少一种） | 阶段名称，如"预选赛"、"正赛日" |
| description_zh / description_en | RichText | 否 | 阶段说明 |
| starts_at | DateTime | 是 | 阶段开始时间 |
| ends_at | DateTime | 是 | 阶段结束时间 |
| session_ids | UUID[] | 是 | 本阶段下的 Session 顺序 |
| eligibility_source | Enum | 否 | `roundRegistration` / `previousStageResult` / `manualInvite` |
| advancement_rule | AdvancementRule | 否 | 晋级规则，如"圈速榜前 30 名进入正赛" |
...
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| source_session_id | UUID | 否 | 以哪个 Session 的成绩作为晋级来源 |
| metric | Enum | 是 | `bestLap` / `points` / `position` / `manual` |
| direction | Enum | 是 | `asc` / `desc`，圈速越快用 `asc` |
| limit | Integer | 否 | 晋级人数或队伍数 |
| target_stage_id | UUID | 否 | 晋级到的下一 Stage |
| fallback_policy | Enum | 否 | 已晋级用户缺席时是否顺延 |
...
| Advancement | Stage | 晋级名单由 Stage 的 advancement_rule 生成，可由管理员复核 |
...
管理员进入后台后统一创建 Competition，再根据赛事形态配置 Round、Stage、Session。单场赛、传统锦标赛、含预选赛的年度赛事都使用同一套编辑器，仅模板不同。

```mermaid
flowchart TD
    A[管理员进入后台<br/>点击'创建赛事'] --> B{选择赛事模板}

    B -->|单场赛| C[创建 Competition<br/>自动生成 1 个 Round]
    B -->|传统锦标赛| D[创建 Competition<br/>批量添加多个 Round]
    B -->|含预选赛分站| E[创建 Competition<br/>每个 Round 内添加预选/正赛 Stage]
...
flowchart TD
    ...
    C --> D[设定赛事形态<br/>single/championship/series]
    D --> E[选择游戏 AC/ACC<br/>车型/公共规则]
    E --> E1[配置默认开服参数<br/>模板/容量/天气/规则]
    E1 --> F[配置 Split/积分/晋级/准入]
...
5. 选择赛事形态：单场赛 / 传统锦标赛 / 年度系列赛
6. 设定游戏平台、车型组、公共赛制参数、默认天气、默认是否进站；MVP 只允许选择 AC / ACC
7. 配置默认开服参数：
...
8. 配置多 Split、积分规则、晋级规则、准入条件
   - 是否启用自动 Split
   - 单服务器最大人数
   - 分组规则（按实力/随机/手动/先到先得）
9. 选择发布区域（CN / AP / AM / EU，可多选或全选）
10. 配置资源下载、直播链接（可选）
11. 进入 Round 编排器
...
flowchart TD
    ...
    C --> E[添加 Stage]
    E --> F{Stage 类型}
    F -->|预选赛| G[配置时间窗口<br/>晋级规则/榜单来源]
    F -->|正赛日| H[配置 Practice/Qualifying/Race]
    F -->|决赛| I[配置 Race Session]
    F -->|自定义| J[自由配置名称和规则]

    G --> K[添加 Session<br/>时间/结果/开服配置]
    H --> K
    I --> K
    J --> K
    K --> L{继续添加 Stage?}
...
2. 填写 Round 名称、赛道/布局、报名开始/截止、取消报名截止时间
3. 如是长期锦标赛，可批量生成多个 Round，并逐站补充赛道
4. 为 Round 添加 Stage：
   - 预选赛：设置开放时间、服务器重启间隔、结果合并规则、晋级人数和榜单指标
   - 正赛日：设置正式比赛流程，通常包含 Practice / Qualifying / Race
   - 决赛：设置 Race Session 或多场决赛
   - 自定义：用于复活赛、安慰赛、表演赛等特殊流程
...
- 传统锦标赛模板：生成多个 Round，每个 Round 复用相同 Stage/Session 结构
- 长时间预选锦标赛模板：每个 Round 默认包含"预选赛"Stage（长时间开放 practice Session）和"正赛日"Stage（Practice / Qualifying / Race）
...
赛事列表统一展示 Competition 卡片，不再区分底层的单场赛卡片和锦标赛卡片。
...
平台统一使用 Competition 详情页。单场赛、传统锦标赛、含预选赛的长期赛事都通过同一个页面结构展示，只是 Round/Stage 数量不同。
...
> **v3.0-draft 变更摘要**：本副本将赛事核心结构从 Event / Championship 双模型调整为 `Competition → Round → Stage → Session`。单场赛、传统锦标赛和含预选赛的年度赛事统一使用 Competition；每一站为 Round，站内流程为 Stage，具体服务器和成绩归属为 Session。

> **v3.1-draft 变更摘要**：明确 MVP 阶段赛事仅支持 AC / ACC；新增 Session 自动开服配置模型，补充 ACC 七个 Dedicated Server 配置文件字段与 AC 两个核心 INI 配置文件字段，并说明后台字段到前台服务器信息展示的复用边界。
