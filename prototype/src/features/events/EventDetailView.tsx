...
export function EventHeader({ event, title }: { event: SimEvent; title: string }) {
  return (
...
export function EventDetailView({
  ...
  const desc = field(event, 'description')
  const eventRules = field(event, 'rules')
  const scoringRules = field(event, 'scoringRules')
  const hasScoringContent = scoringRules || (event.scoringTable && event.scoringTable.length > 0)
  const eventResources = field(event, 'resources')
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 bg-destructive/10 border border-destructive/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {t('eventDetail.cancelReason')}: {field(event, 'cancelledReason')}
            </p>
          </div>
        </div>
      )}

      <EventHeader event={event} title={name} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div><div className="text-xs text-muted-foreground">{t('eventDetail.pitstop')}</div><div className="text-sm font-medium">{effectiveHasPitstop ? text('需要', 'Yes') : text('不需要', 'No')}</div></div>
              </div>
              <div className="flex items-center gap-3">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-accent rounded-lg p-3">
                ...
                <div className="font-bold">{effectiveRaceDuration} {effectiveRaceDurationType === 'time' ? text('分钟', 'min') : text('圈', 'laps')}</div>
              </div>
            </div>
          </div>
          ...
          {eventResources && (
            <div className="bg-card border border-border rounded-xl p-5">
              <h2 className="font-bold mb-3 flex items-center gap-2"><Download className="w-4 h-4 text-primary" />{text('资源下载 / Resources', 'Resources')}</h2>
