...
import type { ChampionshipDetailData, ChampionshipTab } from '@/features/championships/hooks'
import { StatusBadge } from '@/components/StatusBadge'
import { ScoringRulesCard } from '@/components/ScoringRulesCard'
import { cn } from '@/lib/utils'
import { getEventStatus, type SimEvent } from '@/domain/events'
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <span className="truncate">
              {(() => { const ev = eventsWithResults.find(e => e.id === resultsEventFilter); return ev ? field(ev, 'name') : '' })()}
            </span>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-muted-foreground text-xs border-b border-border bg-accent/50">
                  ...
                  <th className="text-left py-3 px-3 hidden md:table-cell">{text('车队', 'Team')}</th>
...
export function ChampionshipHeader({
  ...
  championship: NonNullable<ChampionshipDetailData['championship']>
  label: string
  statusLabel: string
  title: string
}) {
  return (
    <div className="relative rounded-2xl overflow-hidden h-48 md:h-56 mb-8" style={{ background: getCoverGradient(championship.id) }}>
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
        <div className="flex items-center gap-2 mb-2">
          <Trophy className="w-5 h-5 text-yellow-400" />
          <span className="text-yellow-400 text-sm font-semibold">{label}</span>
          <StatusBadge
            status={championship.status === 'upcoming' ? 'Draft' : championship.status === 'active' ? 'RegistrationOpen' : 'Completed'}
            label={statusLabel}
          />
        </div>
...
export function ChampionshipDetailView({
  ...
  detail: ChampionshipDetailData
  activeTab: ChampionshipTab
  routeEventId: string
  setRouteState: (next: { tab?: ChampionshipTab; event?: string | null }) => void
  drivers: Driver[]
  teams: Team[]
}) {
  ...
  const effectiveResultsEventFilter = resultsEventFilter || routeEventId || (eventsWithResults.length > 0 ? eventsWithResults[0].id : '')
  ...
  const rules = field(ch, 'rules')
  const resources = field(ch, 'resources')
  const progression = field(ch, 'progressionRules')
  const chAccessReq = field(ch, 'accessRequirements')
  const scoringRules = field(ch, 'scoringRules')
  ...
  const handleRegister = (event: SimEvent) => {
    if (!state.isLoggedIn) return
    const eventRules = field(event, 'rules')
    const chRules = rules
    const eventAccessReq = field(event, 'accessRequirements')
  ...
  const handleUnregister = (event: SimEvent) => {
    unregister(event)
  }

  const renderResultsSummary = (event: SimEvent) => {
  ...
  const renderEventRow = (event: SimEvent, isPast = false) => {
    ...
    return (
      <div key={event.id} className={cn(
        ...
        routeEventId === event.id && 'border-primary/50 ring-1 ring-primary/30',
      )}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-semibold text-sm">{field(event, 'name')}</h4>
            </div>
        ...
        {isPast && event.results && event.results.length > 0 && (
          <div className="mt-3">
            <button
              onClick={() => { setResultsEventFilter(event.id); setRouteState({ tab: 'results', event: event.id }) }}
              className="text-xs text-primary hover:underline flex items-center gap-1"
            >
              <BarChart3 className="w-3 h-3" />
              {text('查看成绩', 'View results')}
            </button>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 mt-3">
            {event.vodUrl && (
              <a href={event.vodUrl} className="flex items-center gap-1 text-xs text-primary hover:underline">
                <Play className="w-3 h-3" /> {text('直播回放', 'Live Replay')}
              </a>
            )}
  ...
  const tabs: { key: ChampionshipTab; label: string; icon: typeof Trophy }[] = [
    { key: 'info', label: text('锦标赛信息', 'Championship Info'), icon: Trophy },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 overflow-x-visible">
      <ChampionshipHeader
        championship={ch}
        label={text('锦标赛', 'Championship')}
        statusLabel={ch.status === 'upcoming' ? text('即将开始', 'Upcoming') : ch.status === 'active' ? text('进行中', 'Active') : text('已结束', 'Completed')}
        title={field(ch, 'name')}
      />
      ...
      <div className="flex gap-2 mb-6">
        {tabs.map(tab => (
          <button
            key={tab.key}
            onClick={() => setRouteState({ tab: tab.key })}
            className={cn(
      ...
      <div className={cn('grid gap-8', activeTab === 'results' ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3')}>
        <div className={cn('space-y-6', activeTab !== 'results' && 'lg:col-span-2')}>
          {activeTab === 'results' ? (
            <ResultsTab
              ...
              setResultsEventFilter={value => {
                setResultsEventFilter(value)
                setRouteState({ tab: 'results', event: value })
              }}
            ...
            <>
              <div className="bg-card border border-border rounded-xl p-5">
                <p className="text-sm text-muted-foreground">{field(ch, 'description')}</p>
              </div>

              <div className="bg-card border border-border rounded-xl p-5">
                <h2 className="font-bold mb-4">{text('赛事信息', 'Event Info')}</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="text-xs text-muted-foreground">{t('eventDetail.pitstop')}</div><div className="text-sm font-medium">{ch.hasPitstop ? text('需要', 'Yes') : text('不需要', 'No')}</div></div>
                  </div>
                  {ch.practiceDuration && (
                    <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="text-xs text-muted-foreground">{t('eventDetail.race')}</div><div className="text-sm font-medium">{ch.raceDuration} {ch.raceDurationType === 'time' ? text('分钟', 'min') : text('圈', 'laps')}</div></div>
                  </div>
                </div>
              </div>
              ...
              {(scoringRules || (ch.scoringTable && ch.scoringTable.length > 0)) && (
                <div className="bg-card border border-border rounded-xl p-5">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-muted-foreground text-xs border-b border-border">
                          ...
                          <th className="text-left py-2 pr-3 hidden sm:table-cell">{text('车队', 'Team')}</th>
            ...
            <div className="space-y-6">
              ...
              {futureEvents.length > 0 && (
                <CollapsibleSection
                  title={`${text('未来赛事', 'Upcoming')} (${futureEvents.length})`}
                  defaultOpen
        ...
        <div className="space-y-4 sticky top-20">
          {nextRegistrable && (
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="font-bold mb-3">{text('下一场赛事', 'Next Event')}</h3>
              <div className="space-y-2 text-sm mb-4">
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{field(nextRegistrable, 'name')}</span>
                </div>
          ...
          {nextRegistered && nextRegistrable && (
