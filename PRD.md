...
> **v3.0 变更摘要**：赛事核心结构从 Event / Championship 双模型调整为 `Competition → Round → Stage → Session`。单场赛、传统锦标赛和含预选赛的年度赛事统一使用 Competition；每一站为 Round，站内流程为 Stage，具体服务器和成绩归属为 Session。
